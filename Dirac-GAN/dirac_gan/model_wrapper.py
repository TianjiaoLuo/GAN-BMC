...
class ModelWrapper(object):
    ...
    def __init__(self,
        ...
        self.use_brown_motion: bool = use_brown_motion
    ...
    def train(self, instance_noise: bool = True) -> torch.Tensor:
        ...
        for iteration in range(HYPERPARAMETERS["training_iterations"]):
            ...
            # get brown motion
            phi = self.generator.parameter.data
            theta = self.discriminator.linear_layer.weight.data
            brown1 = torch.normal(torch.zeros(phi.shape), torch.ones(phi.shape))
            brown2 = torch.normal(torch.zeros(phi.shape), torch.ones(phi.shape))
            rou1 = HYPERPARAMETERS["rou1"]
            rou2 = HYPERPARAMETERS["rou2"]
            beta = HYPERPARAMETERS["beta"]
            d_phi = rou1 * phi * brown1 - rou2 * phi * torch.pow(torch.abs(phi), beta) * brown2
            d_theta = rou1 * theta * brown1 - rou2 * theta * torch.pow(torch.abs(theta), beta) * brown2
            ...
            # add brown
            if self.use_brown_motion:
                self.generator.parameter.data += d_phi
                self.discriminator.linear_layer.weight.data += d_theta
            # Save parameters
